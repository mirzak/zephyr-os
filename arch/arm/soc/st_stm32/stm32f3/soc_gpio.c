...
static uint32_t __func_to_mode(int func)
{
	...
	case STM32F3X_PIN_CONFIG_DRIVE_PUSH_UP:
	case STM32F3X_PIN_CONFIG_DRIVE_PUSH_DOWN:
	case STM32F3X_PIN_CONFIG_DRIVE_OPEN_DRAIN:
	case STM32F3X_PIN_CONFIG_DRIVE_OPEN_UP:
	case STM32F3X_PIN_CONFIG_DRIVE_OPEN_DOWN:
		return 0x1;
	case STM32F3X_PIN_CONFIG_AF_PUSH_PULL:
	case STM32F3X_PIN_CONFIG_AF_PUSH_UP:
	case STM32F3X_PIN_CONFIG_AF_PUSH_DOWN:
	case STM32F3X_PIN_CONFIG_AF_OPEN_DRAIN:
	case STM32F3X_PIN_CONFIG_AF_OPEN_UP:
	case STM32F3X_PIN_CONFIG_AF_OPEN_DOWN:
		return 0x2;
...
int stm32_gpio_flags_to_conf(int flags, int *pincfg)
{
	...
	if (direction == GPIO_DIR_OUT) {
		if (pud == GPIO_PUD_PULL_UP) {
			*pincfg = STM32F3X_PIN_CONFIG_DRIVE_PUSH_UP;
		} else if (pud == GPIO_PUD_PULL_DOWN) {
			*pincfg = STM32F3X_PIN_CONFIG_DRIVE_PUSH_DOWN;
		} else {
