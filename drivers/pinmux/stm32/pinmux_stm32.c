...
static int enable_port(uint32_t port, struct device *clk)
{
	...
	return clock_control_on(clk, subsys);
        ...
        struct stm32f3x_pclken pclken;
        uint32_t ports_enable[STM32_PORTS_MAX] = {
                STM32F3X_CLOCK_ENABLE_GPIOA,
                STM32F3X_CLOCK_ENABLE_GPIOB,
                STM32F3X_CLOCK_ENABLE_GPIOC,
                STM32F3X_CLOCK_ENABLE_GPIOD,
                STM32F3X_CLOCK_ENABLE_GPIOE,
                STM32F3X_CLOCK_ENABLE_GPIOF,
                STM32F3X_CLOCK_ENABLE_GPIOG,
                STM32F3X_CLOCK_ENABLE_GPIOH,
        };
	/* AHB2 bus for all the GPIO ports */
	pclken.bus = STM32F3X_CLOCK_BUS_AHB2;
...
static int stm32_pin_configure(int pin, int func, int altf)
{
	 ...
	 */
	return stm32_gpio_configure((uint32_t *)port_base, STM32_PIN(pin), func, altf);
}
