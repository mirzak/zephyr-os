...
static inline int stm32f3x_clock_control_on(struct device *dev,
					     clock_control_subsys_t sub_system)
{
	struct stm32f3x_rcc_data *data = dev->driver_data;
	volatile struct stm32f3x_rcc *rcc = (struct stm32f3x_rcc *)(data->base);
	struct stm32f3x_pclken *pclken = (struct stm32f3x_pclken *)(sub_system);
	uint32_t tmpreg;

	switch (pclken->bus) {
	case STM32F3X_CLOCK_BUS_AHB1:
	case STM32F3X_CLOCK_BUS_AHB2:
	case STM32F3X_CLOCK_BUS_AHB3:
		tmpreg = rcc->ahbenr;
		tmpreg |= pclken->enr;
		rcc->ahbenr = tmpreg;
		break;
	case STM32F3X_CLOCK_BUS_APB1:
		rcc->apb1enr |= pclken->enr;
		break;
	case STM32F3X_CLOCK_BUS_APB2:
...
static inline int stm32f3x_clock_control_off(struct device *dev,
					      clock_control_subsys_t sub_system)
{
	struct stm32f3x_rcc_data *data = dev->driver_data;
	volatile struct stm32f3x_rcc *rcc = (struct stm32f3x_rcc *)(data->base);
...
{
	if (prescaler == 0) {
		return STM32F3X_RCC_CFG_HCLK_DIV_0;
	}

	const struct regval_map map[] = {
		{0,	STM32F3X_RCC_CFG_HCLK_DIV_0},
		{2,	STM32F3X_RCC_CFG_HCLK_DIV_2},
		{4,	STM32F3X_RCC_CFG_HCLK_DIV_4},
		{8,	STM32F3X_RCC_CFG_HCLK_DIV_8},
		{16,	STM32F3X_RCC_CFG_HCLK_DIV_16},
	};
...
{
	if (prescaler == 0)
		return STM32F3X_RCC_CFG_SYSCLK_DIV_0;

	const struct regval_map map[] = {
		{0,	STM32F3X_RCC_CFG_SYSCLK_DIV_0},
		{2,	STM32F3X_RCC_CFG_SYSCLK_DIV_2},
		{4,	STM32F3X_RCC_CFG_SYSCLK_DIV_4},
		{8,	STM32F3X_RCC_CFG_SYSCLK_DIV_8},
		{16,	STM32F3X_RCC_CFG_SYSCLK_DIV_16},
		{64,	STM32F3X_RCC_CFG_SYSCLK_DIV_64},
		{128,	STM32F3X_RCC_CFG_SYSCLK_DIV_128},
		{256,	STM32F3X_RCC_CFG_SYSCLK_DIV_256},
		{512,	STM32F3X_RCC_CFG_SYSCLK_DIV_512},
	};
	return map_reg_val(map, ARRAY_SIZE(map), prescaler);
}
...
static int stm32f3x_clock_control_get_subsys_rate(struct device *clock,
						   clock_control_subsys_t sub_system,
						   uint32_t *rate)
{
	ARG_UNUSED(clock);
	...
	uint32_t apb1_clock = __get_apb_clock(ahb_clock,
				CONFIG_CLOCK_STM32F3X_APB1_PRESCALER);
	uint32_t apb2_clock = __get_apb_clock(ahb_clock,
				CONFIG_CLOCK_STM32F3X_APB2_PRESCALER);
...
static struct clock_control_driver_api stm32f3x_clock_control_api = {
...
{
	volatile struct stm32f3x_flash *flash =
		(struct stm32f3x_flash *)(FLASH_BASE);

	if (CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC <= 24000000) {
		flash->acr.bit.latency = STM32F3X_FLASH_LATENCY_0;
	} else if (CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC <= 48000000) {
		flash->acr.bit.latency = STM32F3X_FLASH_LATENCY_1;
	} else if (CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC <= 72000000) {
		flash->acr.bit.latency = STM32F3X_FLASH_LATENCY_2;
	}
}

int stm32f3x_clock_control_init(struct device *dev)
{
	struct stm32f3x_rcc_data *data = dev->driver_data;
	volatile struct stm32f3x_rcc *rcc =
		(struct stm32f3x_rcc *)(data->base);
	/* SYSCLK source defaults to HSI */
	int sysclk_src = STM32F3X_RCC_CFG_SYSCLK_SRC_HSI;
	uint32_t hpre = __ahb_prescaler(CONFIG_CLOCK_STM32F3X_AHB_PRESCALER);
	uint32_t ppre1 = __apb_prescaler(CONFIG_CLOCK_STM32F3X_APB1_PRESCALER);
	uint32_t ppre2 = __apb_prescaler(CONFIG_CLOCK_STM32F3X_APB2_PRESCALER);
#ifdef CONFIG_CLOCK_STM32F3X_PLL_MULTIPLIER
	uint32_t pllmul = __pllmul(CONFIG_CLOCK_STM32F3X_PLL_MULTIPLIER);
#endif	/* CONFIG_CLOCK_STM32F3X_PLL_MULTIPLIER */
	...
	/* PLL input from HSI */
	rcc->cfgr.bit.pllsrc = STM32F3X_RCC_CFG_PLL_SRC_HSI;
	...
	rcc->cfgr.bit.pllsrc = STM32F3X_RCC_CFG_PLL_SRC_HSE;
...
#ifdef CONFIG_CLOCK_STM32F3X_SYSCLK_SRC_PLL
	...
	sysclk_src = STM32F3X_RCC_CFG_SYSCLK_SRC_PLL;
	...
	sysclk_src = STM32F3X_RCC_CFG_SYSCLK_SRC_HSE;
#endif
